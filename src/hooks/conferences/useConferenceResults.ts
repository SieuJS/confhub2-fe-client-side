...
const useConferenceResults = ({ initialData, initialTotalItems }: UseConferenceResultsProps = {}) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            queryParams.append('page', page); // Keep page for pagination
            queryParams.append('sortBy', sortByParam); // Keep sortBy for server-side sorting
            queryParams.append('limit', String(eventsPerPage)); // IMPORTANT: Tell the API how many items per page
            ...
            // Assuming the API returns an object like { items: ConferenceInfo[], total: number }
            setEvents(data.items);        //  Set the *current page* of events
            setTotalItems(data.total);    //  Set the *total* number of events
    ...
    //  NO client-side sorting.  The API handles sorting.
    // const sortedEvents = sortEvents(currentEvents, sortBy); // REMOVE THIS LINE

    //  Use the 'events' directly.  This is the current page of data.
    const sortedEvents = events;
    return {
      sortedEvents, // This is now the current page of data, already sorted by the API
      totalItems,
      eventsPerPage,
      currentPage,
      sortBy,
