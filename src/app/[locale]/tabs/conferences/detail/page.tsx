...
const Detail: React.FC<EventCardProps> = ({ locale }: EventCardProps) => {
  ...
  useEffect(() => {
    async function getInfoConference() {
      try {
        // Use the imported getConference function.
  ...
  const handleShareClick = (platform: 'facebook' | 'twitter' | 'reddit') => {
    if (!conferenceData || !conferenceData.payload || conferenceData.payload.length === 0) {
      return; // Or handle the case where conferenceData is not available
    }

    const conference = conferenceData.payload[0];
    const conferenceDates = conference.dates; // Find main conference dates

    // Extract date strings from conferenceDates
    let fromDateStr = '';
    let toDateStr = '';

    if (conferenceDates && conferenceDates.length > 0) {
      const conferenceDate = conferenceDates.find(date => date.name === 'conferenceDates');
      if (conferenceDate) {
        fromDateStr = conferenceDate.fromDate;
        toDateStr = conferenceDate.toDate;
      }
    }
    ...
    const shareText = `📢 ${conference.title}\n📅 Thời gian: ${fromDateFormatted} - ${toDateFormatted} \n📍 Địa điểm: ${conference.location.cityStateProvince}, ${conference.location.country}\n🔗 Chi tiết: ${conference.link}`;
    const shareUrl = encodeURIComponent(conference.link || window.location.href);
  ...
  useEffect(() => {
    async function getInfoConference() {
      try {
        const conferenceInfo = await getConference(acronym);
        setConferenceData(conferenceInfo);
      } catch (err: any) {
        setError(err.message || 'An error occurred during update.');
        console.error('Error updating conference data:', err);
  ...
  const displayedTopics = showAllTopics ? conferenceData?.payload?.[0]?.topics || [] : (conferenceData?.payload?.[0]?.topics || []).slice(0, 10);
  const hasMoreTopics = (conferenceData?.payload?.[0]?.topics || []).length > 10;

  const conference = conferenceData?.payload?.[0];
  if (!conference) return <NotFoundPage />;

  const conferenceDates = conference.dates;
  let fromDateStr = '';
  let toDateStr = '';
  if (conferenceDates && conferenceDates.length > 0) {
    const conferenceDate = conferenceDates.find(date => date.name === 'conferenceDates');
  ...
  return (
    <div>
      ...
      <div className='relative'>
        ...
        {conferenceData && conferenceData.payload && conferenceData.payload.length > 0 && (
          <div className='px-10'>
            <div>
              <div className="py-12 w-full"></div> {/* Đã thay bg-background thành bg-gray-100 */}
              <div className="container mx-auto py-6 px-4 rounded-lg flex flex-col md:flex-row gap-6">
                  ...
                  <div className="relative">
                    <div className="flex mt-4">
                      ...
                      <div className="flex-1">
                        <p className="text-red-500 text-sm">{dateDisplay}</p> {/* Dates in red color */}
                ...
                <div className="md:w-1/5 ">
                  <div className="flex flex-col items-end">
                    ...
                    <div className="flex flex-row items-center w-full justify-end space-x-2"> {/* Flex row container for Calendar and Share buttons, w-full and justify-end and space-x-2 for spacing */}
                      <div className="">
                        <Button
                            ...
                            size="medium"
                            rounded
                            className="hover:opacity-90 p-2 flex items-center max-h-10" // Added flex and items-center to button class
                        >
                            {/* Inline SVG for Calendar Icon */}
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6 mr-2"> {/* Đã thêm mr-2 vào svg để tạo khoảng cách */}
                    ...
                    <div className="mt-6 flex flex-col gap-2 w-full"> {/* w-full for full width buttons */}
                      <Button
                        onClick={handleFollowClick}
                        variant={isFollowing ? 'primary' : 'secondary'}
                        size="medium"
                        rounded
                        className={`hover:opacity-90 w-full`} // w-full to take full width
                      >
                      ...
                      <Button
                        ...
                        size="medium"
                        rounded
                        className={`hover:opacity-90 w-full ${updating ? '' : ''}`} // w-full to take full width, removed conditional width
                      >
                        {updating ? 'Updating' : 'Update'}
                      </Button>
                      <Button
                        onClick={() => {
                          if (conferenceData?.payload?.[0]?.link) {
                            window.open(conferenceData.payload[0].link, '_blank');
                          } else {
                            // console.warn("Website link is not available.");
