...
const Detail: React.FC<EventCardProps> = ({ locale }: { locale: string }) => {
  ...
  useEffect(() => {
    async function getInfoConference() {
        ...
        const conferenceInfo = await getConference(id); // false to use API, true to use client-side data
    ...
    const handleShareClick = (platform: 'facebook' | 'twitter' | 'reddit') => {
        const conferenceDates = conferenceData.conferenceDates; // Find main conference dates
    ...
    async function handleUpdateClick () {
      ...
      let fetchedData: Conference;
         try {
          ...
          newConferenceInfo.conference[0].acronym = conferenceData.conference[0].acronym;
          newConferenceInfo.conference[0].title = conferenceData.conference[0].title;
          newConferenceInfo.organization[0].link = conferenceData.organization[0].link;
          newConferenceInfo.organization[0].impLink = conferenceData.organization[0].impLink;
          newConferenceInfo.organization[0].cfpLink = conferenceData.organization[0].cfpLink;
    ...
    const toggleShareMenu = () => {
      console.log(conferenceData?.ranks[0])
        ...
        {conferenceData && 
          ...
          <div>
            ...
            <Image
              ...
              alt={`${conferenceData?.conference?.title} - secondary`}
              width={800}
              height={400}
            ...
            <p className="text-lg">
            {/* {conferenceData?.dates?.[0] &&
              conferenceData.dates[0].findIndex((date) => date.name === 'conferenceDates') !== -1
                ? new Date(conferenceData.dates[0].name?.fromDate).toLocaleDateString()
                : ''} - {conferenceData?.dates?.[0] &&
                  conferenceData.dates[0].dates.findIndex((date) => date.name === 'conferenceDates') !== -1
                    ? new Date(conferenceData.dates[0].dates.find((date) => date.name === 'conferenceDates')?.toDate).toLocaleDateString()
                    : ''} */}
            </p>
            <div>
              <span>{conferenceData?.conference?.acronym}</span> {/* shortName to acronym */}
            </div>
            </div>

            <h2 className=" font-bold text-left text-4xl mt-2">{conferenceData?.conference?.title}</h2>

            <p className="text-left mt-4">{conferenceData?.organization?.summerize}</p>

            <div className="flex items-center mt-6 text-left">
            <span className="font-semibold mr-2 pb-2">Topics:</span>
            <div className="flex flex-wrap">
              {conferenceData?.organization?.topics?.map((topic, index) => (
          ...
          <table className="w-full text-md text-left border-collapse">
            ...
            <tbody>
              <tr className="border-b  ">
                ...
                {/* {conferenceData?.dates?.[0] &&
              conferenceData.dates[0].findIndex((date) => date.name === 'conferenceDates') !== -1
                ? new Date(conferenceData.dates[0].find((date) => date.name === 'conferenceDates').fromDate).toLocaleDateString()
                : 'Dates not available'} */}
                </td>
              </tr>
              ...
              <tr className="border-b  ">
                ...
                {/* {conferenceData?.dates?.[0] &&
              conferenceData.dates[0].findIndex((date) => date.name === 'conferenceDates') !== -1
                ? new Date(conferenceData.dates[0].find((date) => date.name === 'conferenceDates').toDate).toLocaleDateString()
                : 'Dates not available'} */}
                </td>
              </tr>
              ...
              <tr className="border-b  ">
                ...
                <td className="px-3 py-2">{conferenceData?.locations?.[0]?.country}</td>
              </tr>
              ...
              <tr className="border-b  ">
                ...
                <td className="px-3 py-2">
                  <a
                    href={conferenceData?.organization?.link} // website to link
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:underline"
                  >
                    {conferenceData?.organization?.link === null ? '' : conferenceData?.organization?.link} {/* website to link */}
                  </a>
                </td>
              </tr>
              ...
              <tr className="border-b  ">
                ...
                <td className="px-3 py-2">{conferenceData?.ranks?.[0]?.byRank?.[0]?.value}</td>
              </tr>

              <tr className="border-b  ">
                <td className="px-3 py-2 font-semibold">{t('Type')}</td>
                <td className="px-3 py-2" >{conferenceData?.organization?.accessType}</td>
              </tr>

              <tr className="border-b  ">
                <td className="px-3 py-2 font-semibold">{t('Field')}</td>
                <td className="px-3 py-2">{conferenceData?.ranks?.[0]?.inFieldOfResearch?.name}</td> {/* fieldOfResearch to category */}
              </tr>
            </tbody>
          </table>
            </section>
