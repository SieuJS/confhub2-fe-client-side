      ...
      try {
        ...
        notesWithLocation.forEach(note => {
          ...
          if (!groupedNotes[key]) {
            ...
            const dateExists = groupedNotes[key].dates.some(
              (d: any) =>
                d.year === note.year &&
                d.month === note.month &&
                d.day === note.day
          ...
          .map((note: any) => {
            ...
            if (note.dates.length > 0) {
              ...
              if (note.dates.length === 1) {
                formattedDateRange = firstDateObj.toLocaleDateString(
                  'en-US',
                  options
                )
              } else if (
  ...
  return (
    <div className='flex h-full flex-col bg-background p-4'>
      <div className='mb-4 flex flex-col gap-4 lg:flex-row'>
          ...
          {upcomingNotes.length === 0 ? (
            ...
            <div className='flex flex-row flex-wrap gap-4'>
              {upcomingNotes.map((note, index) => (
                <div
                  key={index}
                  className={`w-full rounded-md border p-4 shadow-md sm:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1rem)] ${note.types.map(getEventTypeColor).join(' ')}`}
                >
                  <div className='flex h-full flex-col'>
                    ...
                    <div className='h-3/4'>
                      ...
                      <div className='mt-1 flex items-center '>
                        ...
                        {/* Location on its own line, smaller text */}
                      </div>
                      <p className='mt-1 text-sm '>
                        {' '}
                        {/* Date on its own line, smaller text */}
                        {note.date}
                      </p>
                      ...
                      <div className='mt-2 flex flex-wrap gap-1'>
                        {note.types.map((type: NoteType, typeIndex: number) => {
                          ...
                          return (
                            <span
                              key={typeIndex}
                              className={`rounded-full px-2 py-1 text-xs font-semibold ${colorClasses}`}
                            >
                              {typeText}
                            </span>
                          )
        ...
        <section className='rounded-md bg-background px-4 pb-6 pt-4 shadow lg:w-1/6'>
          ...
          <ul>
            ...
            <li className='mb-4 flex items-center'>
              <div className='mr-2 h-4 w-4 rounded-full bg-button'></div>
