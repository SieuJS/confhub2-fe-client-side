...
const MyConferencesTab: React.FC = () => {
  ...
  const { user, isLoggedIn, isInitializing: isAuthInitializing, getToken } = useAuth();
  ...
  const transformedConferences = useMemo(() => {
    ...
    return conferences
      .map((conf) => ({
        ...
        fromDate: conf.organizations?.[0]?.conferenceDates?.find((d) => d.type === 'conferenceDates')?.fromDate,
        toDate: conf.organizations?.[0]?.conferenceDates?.find((d) => d.type === 'conferenceDates')?.toDate,
  ...
  if (conferencesError) {
    return (
      <div className="flex h-60 flex-col items-center justify-center">
        <p className="mb-2 text-red-500">{t('Error_loading_conferences')}: {conferencesError}</p>
  ...
  const getStatusTitle = (status: ConferenceStatus | 'All') => {
    ...
    switch (status) {
      case ConferenceStatus.Approve: return t('My_conferences_are_approved');
      case ConferenceStatus.Pending: return t('My_conferences_are_pending');
      case ConferenceStatus.Rejected: return t('My_conferences_are_rejected');
      default: return t('My_conferences'); // Fallback title
    }
  };
    ...
    <div className='container mx-auto p-4'>
      ...
      {filteredConferences.length === 0 ? (
        <div className="my-10 text-center text-gray-500">
          <p className='text-lg'>{t('You_have_no_conference_in_this_category')}</p>
          {displayStatus !== 'All' && (
            <Button
              variant="primary"
              onClick={() => setDisplayStatus('All')}
        ...
        <div className="space-y-6">
          {filteredConferences.map((conference) => (
            <div
              className='rounded-lg border bg-white p-4 shadow-md transition-shadow hover:shadow-lg' // Cải thiện styling
              key={conference.id}
            >
              ...
              <ConferenceItem
                conference={{ // Chỉ truyền những gì ConferenceItem cần
                  id: conference.id,
