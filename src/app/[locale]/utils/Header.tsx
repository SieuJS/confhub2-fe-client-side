...
export const Header: FC<Props> = ({ locale }) => {
  ...
  const CloseIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="text-foreground h-8 w-8">
  ...
  // --- Dropdown Components (using conditional rendering) ---
    const NotificationDropdown = () => {
        return (
            <div
                className={`absolute right-0 top-full mt-2 w-80 bg-background-secondary border border-border rounded-md shadow-lg z-50 ${isNotificationOpen ? '' : 'hidden'
                    }`}
            >
                <div className="py-2">
          ...
          <div className="px-4 py-2 text-sm text-foreground hover:bg-button/10 text-center border-t border-border">
            <Link href="/dashboard" lang={locale} onClick={closeAllMenus} className="text-button hover:underline block">
    ...
    return (
      <div className={`border-border absolute left-0 right-0 top-full z-40 border-b bg-background-secondary shadow-md sm:hidden ${isMobileMenuOpen ? '' : 'hidden'}`}>
    ...
    >
      ...
      <div className='relative flex flex-row items-center gap-3'>
        ...
        <nav className='mr-10 hidden gap-5 sm:inline-flex'>
          ...
          {!isClient ? (  // Show loading indicator
              <div className=''>
                  <div className='flex items-center'>
                      <svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none' stroke='#ffffff' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' className='lucide lucide-log-in mr-2 animate-spin'>
                          <path d='M12 22s8-4 8-10V4' />
                            <path d='M12 2s8 4 8 10'/>
                      </svg>
                  </div>
              </div>
                    ) : (
