...
const GeneralPagination: React.FC<PaginationProps> = ({
  ...
  const buttonBaseClass =
    'min-w-[32px] h-8 flex items-center justify-center px-3 text-sm rounded-md duration-200 ease-in-out'
  const buttonInactiveClass =
    'bg-white-pure  hover:bg-gray-10 border border-gray-300'
  const buttonActiveClass =
    'bg-blue-600 text-white border border-blue-600 shadow-md cursor-default'
  const buttonDisabledClass =
    'bg-gray-10  border border-gray-200 cursor-not-allowed pointer-events-none' // pointer-events-none for full disable
  ...
  return (
    ...
    >
      <ul className='inline-flex items-center space-x-2'>
        {' '}
        {/* Use space-x for modern spacing */}
        {/* Previous Button */}
        <li>
          <button
