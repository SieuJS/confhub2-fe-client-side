...
const NotificationDropdown: FC<Props> = ({
  ...
  // Nhóm thông báo
  const groupNotifications = useCallback((notificationsToGroup: Notification[]) => {
    const now = new Date();
    const oneDayAgo = now.getTime() - 24 * 60 * 60 * 1000;
    const newNotificationsGroup = notificationsToGroup.filter(n => new Date(n.createdAt).getTime() >= oneDayAgo);
    const earlierNotificationsGroup = notificationsToGroup.filter(n => new Date(n.createdAt).getTime() < oneDayAgo);
    return { newNotifications: newNotificationsGroup, earlierNotifications: earlierNotificationsGroup };
  }, []);

  const { newNotifications, earlierNotifications } = groupNotifications(sortedNotifications);

  // Render một item thông báo
  const renderNotificationItem = useCallback((notification: Notification) => {
    const sanitizedMessage = typeof window !== 'undefined' ? DOMPurify.sanitize(notification.message) : notification.message;
    return (
      <Link
      ...
      >
        <div className={`flex items-start border-b border-gray-20 p-4 hover:bg-blue-50 ${!notification.seenAt ? 'bg-gray-10 dark:bg-gray-700' : ''}`}>
          <div className='mr-3 flex-shrink-0'>
            <div className='relative flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 dark:bg-gray-600'>
  ...
  }, [locale, language, closeAllMenus]); // Bỏ các dependency không cần thiết
  ...
  return (
    <div
      className={`absolute right-0 z-50 mr-8 mt-10 w-80 overflow-hidden rounded-lg bg-white-pure shadow-xl transition-all duration-300 ease-in-out dark:bg-gray-800 dark:border dark:border-gray-700 md:w-[400px] ${isNotificationOpen
      ...
      <div className='border-b border-gray-200 p-4 dark:border-gray-700'>
        <div className='flex items-center justify-between'>
          ...
          <button
            ...
            disabled={isLoadingNotifications || notifications.every(n => n.seenAt)}
          >
            {t('Mark All As Read')}
          </button>
        </div>
