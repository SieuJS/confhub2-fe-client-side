    ...
    <div className="sm:col-span-6">
      ...
      <div className="space-y-6">
        {formData.dates.map((date, index) => {
          ...
          const shouldShowError = (fieldName: keyof typeof errorsForThisDate) => {
            return touchedFields.has(`dates.${index}.${fieldName}`) && !!errorsForThisDate[fieldName];
          };
              ...
              <div className="grid grid-cols-1 gap-y-4 gap-x-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    id={`name-${index}`}
                    value={date.name}
                    onChange={e => handlers.handleDateChange(index, 'name', e.target.value)}
                    ...
                    disabled={index === 0}
                  />
                  {shouldShowError('name') && <p className="mt-1 text-sm text-red-600">{errorsForThisDate.name}</p>}
                </div>
                ...
                <div>
                  <label htmlFor={`type-${index}`} className="flex items-center text-sm font-medium text-gray-700">
                    {index > 0 && <span className="text-red-500">* </span>}
                    {t('Type')}
                    {tooltipText && <InformationCircleIcon className="ml-1.5 h-5 w-5 text-gray-400" />}
                  </label>
                  <select
                    id={`type-${index}`}
                    value={date.type}
                    onChange={e => handlers.handleDateChange(index, 'type', e.target.value)}
                    onBlur={() => handlers.handleBlur(`dates.${index}.type`)} // Gắn onBlur
                    className={clsx(
                      'p-2.5 mt-1 block w-full rounded-md shadow-sm sm:text-sm',
                      shouldShowError('type') ? 'border-red-500' : 'border-gray-300',
                      index === 0 ? 'bg-gray-100 cursor-not-allowed' : 'focus:border-indigo-500 focus:ring-indigo-500'
                    )}
                    required={index > 0}
                    disabled={index === 0}
                  >
                    ...
                    {dateTypeOptions
                      ...
                      .map(opt => <option key={opt.value} value={opt.value}>{opt.name}</option>)}
                  </select>
                  {shouldShowError('type') && <p className="mt-1 text-sm text-red-600">{errorsForThisDate.type}</p>}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    id={`fromDate-${index}`}
                    value={date.fromDate}
                    onChange={e => handlers.handleDateChange(index, 'fromDate', e.target.value)}
                    onBlur={() => handlers.handleBlur(`dates.${index}.fromDate`)} // Gắn onBlur
                    className={clsx(
                      'p-2 mt-1 block w-full rounded-md shadow-sm sm:text-sm focus:border-indigo-500 focus:ring-indigo-500',
                      shouldShowError('fromDate') ? 'border-red-500' : 'border-gray-300'
                    )}
                    required
                  />
                  {shouldShowError('fromDate') && <p className="mt-1 text-sm text-red-600">{errorsForThisDate.fromDate}</p>}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    id={`toDate-${index}`}
                    value={date.toDate}
                    onChange={e => handlers.handleDateChange(index, 'toDate', e.target.value)}
                    onBlur={() => handlers.handleBlur(`dates.${index}.toDate`)} // Gắn onBlur
                    min={date.fromDate}
                    className={clsx(
                      'p-2 mt-1 block w-full rounded-md shadow-sm sm:text-sm focus:border-indigo-500 focus:ring-indigo-500',
                      shouldShowError('toDate') ? 'border-red-500' : 'border-gray-300'
                    )}
                    required
                  />
                  {shouldShowError('toDate') && <p className="mt-1 text-sm text-red-600">{errorsForThisDate.toDate}</p>}
                </div>
              </div>
            </div>
          );
