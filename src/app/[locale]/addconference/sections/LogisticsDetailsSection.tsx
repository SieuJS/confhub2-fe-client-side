...
const LogisticsDetailsSection: React.FC<
  LogisticsDetailsSectionProps
> = props => {
    ...
    <div id={id}>
      ...
      >
              ...
              <button
                ...
                onClick={() => {
                  handlers.handleFieldChange(
                    'type',
                    option.id as 'Online' | 'Offline' | 'Hybrid'
                  )
                  handlers.handleBlur('type') // Coi như click là đã "touch" và "blur"
                }}
                className={clsx(
                  'rounded-md px-3 py-2 text-center text-sm font-medium transition-all duration-150',
                  formData.type === option.id
                    ? 'bg-indigo-600 text-white shadow-sm ring-2 ring-indigo-500 ring-offset-2'
                    : 'bg-white-pure  ring-1 ring-inset ring-gray-300 hover:bg-gray-10',
        ...
        <div className='sm:col-span-6'>
          ...
          <LocationSelector {...props} isRequired={isLocationRequired} />
        </div>
