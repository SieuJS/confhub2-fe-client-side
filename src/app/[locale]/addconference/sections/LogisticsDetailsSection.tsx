...
import clsx from 'clsx'; // Import clsx để quản lý class động
  ...
  return (
    <div id={id}>
      ...
      >
          ...
          <div className="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {typeOptions.map(option => (
              <button
                key={option.id}
                type="button"
                onClick={() => onFieldChange('type', option.id as 'Online' | 'Offline' | 'Hybrid')}
                className={clsx(
        ...
        {/* --- Location Selector --- */}
        <div className="sm:col-span-6">
          {/* Truyền các props cần thiết xuống LocationSelector */}
          <LocationSelector
            {...props} // Truyền tất cả props xuống để nó có thể truy cập formData, errors, handlers,...
