...
const VerticalProgressStepper: React.FC<VerticalProgressStepperProps> = ({
  ...
  return (
    <div className='flex flex-col justify-between p-4'>
            ...
            return (
              <li key={step.name} className='relative'>
                ...
                {stepIdx !== steps.length - 1 ? (
                  <div
                    className={clsx(
                      ...
                      isCompleted ? 'bg-indigo-600' : 'bg-gray-20'
                    )}
                    aria-hidden='true'
                  />
                ) : null}
                <a
                  ...
                  className='group relative -ml-2 flex cursor-pointer items-center rounded-md p-2 transition-colors duration-200 hover:bg-gray-30'
                ...
                >
                  ...
                  >
                    <span
                      className={clsx(
                        'relative z-10 flex h-8 w-8 items-center justify-center rounded-full transition-all duration-300',
                        {
                          ...
                          'border-2 border-indigo-600 bg-white-pure shadow-sm':
                            !isCompleted && isCurrent,
                          'border-2 border-gray-300 bg-white-pure':
                            !isCompleted && !isCurrent
                        },
                    ...
                    >
                      ...
                      {isCompleted ? (
                        <CheckIcon className='h-5 w-5 text-white-pure' />
                      ) : (
      ...
      <div className='mt-10'>
        {isAllComplete ? (
          <div className='rounded-r-lg border-l-4 border-green-400 bg-green-50 p-4 text-center'>
            <SparklesIcon className='mx-auto mb-2 h-8 w-8 text-green-500' />
            <h4 className='text-sm font-bold text-green-800'>
              {t('All_sections_complete')}
            </h4>
            <p className='mt-1 text-xs text-green-700'>
              {t('You_can_now_proceed_to_the_next_step')}
            </p>
          </div>
        ) : (
          <>
            <h4 className='text-sm font-medium '>{t('Form_Completion')}</h4>
            ...
            <div className='mt-3' aria-hidden='true'>
              <div className='overflow-hidden rounded-full bg-gray-20'>
                <div
