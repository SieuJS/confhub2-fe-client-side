...
const AddConferencePage = ({ // Đổi tên để nhất quán (Tùy chọn)
  params: { locale }
}: {
  ...
  useEffect(() => {
    ...
    if (!isAuthInitializing && !isLoggedIn) {
      console.log('[AddConferencePage] Not logged in and auth initialized, redirecting to login.');
    ...
    return (
      <>
        ...
        <main className='container mx-auto min-h-screen px-4 py-8 md:px-16'> {/* Thêm padding và min-h-screen */}
          ...
          <h1 className='mb-6 text-3xl font-bold text-gray-800'>{t('Add_New_Conference')}</h1>
          <ConferenceForm /> {/* Giả sử ConferenceForm không cần locale trực tiếp */}
        </main>
        <Footer />
      </>
    );
