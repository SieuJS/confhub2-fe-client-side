...
import { useSearchParams } // <<< Bỏ useRouter, usePathname nếu không dùng nữa
...
const VerifyEmailForm: React.FC<VerifyEmailFormProps> = (/* { t } */) => { // Nhận t nếu bạn dùng i18n
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_DATABASE_URL}/api/v1/auth/verify`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) { // Status 200
                ...
                // }, 3000);
                // --- KẾT THÚC BỎ ---
            } else { // Lỗi từ backend (400, 404, 500)
    ...
    return (
        <div className='space-y-6'> {/* Đổi form thành div hoặc giữ form nhưng không submit nếu đã thành công */}
            ...
            {!successMessage && (
                <form className='space-y-6' onSubmit={handleSubmit}>
                    ...
                    <div className='text-center text-sm'>
                        ...
                        <span className='text-gray-600'>or</span> {/* Nên dùng t('common.or') */}
                        {' '} {/* Thêm khoảng trắng nếu cần */}
                        <Link href="/auth/login" className='hover:text-button/80 font-medium text-button'>
            ...
            {successMessage && (
                 <div className="mt-6"> {/* Thêm khoảng cách trên nếu cần */}
                     <Link
