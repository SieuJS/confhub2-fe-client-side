...
export default function ChatUI() {
  ...
  const shouldShowExternalStatus =
    showTimer || (connectionStatusMessage && !connected && !isConnecting)

  // Show restart button if disconnected after an attempt (moved from page.tsx)
  const shouldShowRestartButton =
    !connected && streamStartTime !== null && !isConnecting

  return (
    // Main layout div (moved from page.tsx)
    <div className='max-h-3/4  relative flex h-screen flex-col'>
      ...
      {shouldShowRestartButton && (
        ...
        <div className='fixed right-4 top-4 z-50 mt-16 md:right-6 md:top-6'>
          <RestartStreamButton
            onRestart={() => {
              handleReconnect() /* Optional: clear logs if needed after restart */
            }}
          />
        </div>
      )}
