...
export default function MainLayout({
  ...
  const handleModalitySettingChange = (newValue: OutputModality) => { if (!connected) { onModalityChange(newValue); } };
  const handleVoiceSettingChange = (newValue: PrebuiltVoice) => { if (!connected) { onVoiceChange(newValue); } };
  const handleLanguageSettingChange = (newValue: Language) => { if (!connected) { onLanguageChange(newValue); } };


  // --- START: Transform availableLanguages to include flagCode ---
  const languagesWithOptions: LanguageOption[] = availableLanguages.map(lang => ({
  ...
  return (
    <div className="relative flex h-screen flex-row bg-gray-100"> {/* Added relative positioning context */}
      ...
      {!isOutputSettingsOpen && (
        ...
        >
          ...
          <AlignJustify stroke="currentColor" className="h-4 w-4" strokeLinecap="round" strokeLinejoin="round" size={20} strokeWidth={1.5} />
      ...
      <div className={`flex flex-grow flex-col space-y-4 bg-white p-4 text-gray-700 shadow-inner transition-all duration-300 ease-in-out ${isOutputSettingsOpen ? 'ml-72' : 'ml-0' // Adjust margin based on panel state
        }`}>
        ...
        <div className="flex items-center gap-2 rounded-full border border-gray-300 bg-gray-100 p-1.5">
          <ConnectionButton connected={connected} connect={connectWithPermissions} disconnect={handleDisconnect} />
