...
const ModelSelectionDropdown: React.FC<ModelSelectionDropdownProps> = ({
  ...
  const [hoveredModelValue, setHoveredModelValue] = useState<string | null>(null);
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      <button
        type="button"
        className={`flex w-full items-center justify-between rounded-md border bg-white px-3 py-2 text-sm text-gray-700 shadow-sm hover:bg-gray-10 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 ${
      ...
      >
        <span className="flex items-center">
          {CurrentModelIcon && <CurrentModelIcon className="mr-2 h-4 w-4 text-blue-500" />}
          {currentModel.name}
        </span>
        <ChevronDown
          size={16}
          ...
          {availableModels.map((modelItem) => {
            ...
            return (
              ...
              >
                <button
                  type="button"
                  className="flex w-full items-center justify-between px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-200 dark:hover:bg-gray-600 dark:hover:text-white"
                  onClick={() => handleSelectModel(modelItem)}
                  role="option"
                  aria-selected={modelItem.value === currentModel.value}
                >
                  <span className="flex items-center">
                    {ItemIcon && <ItemIcon className="mr-2 h-4 w-4 text-blue-500" />}
