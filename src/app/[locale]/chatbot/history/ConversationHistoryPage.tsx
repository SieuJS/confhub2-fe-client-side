...
const ConversationHistoryPage: React.FC = () => { // Đổi tên component
  ...
  const handleDeleteClick = (id: string, title: string) => {
    if (window.confirm(t('Confirm_Delete_Conversation', { title: title || t('Untitled_Conversation') }))) {
      deleteConversation(id)
    }
  }

  const handleClearClick = (id: string, title: string) => {
    if (window.confirm(t('Confirm_Clear_Conversation', { title: title || t('Untitled_Conversation') }))) {
