...
const ModelInfoTooltip: React.FC<ModelInfoTooltipProps> = ({ model, isVisible, targetRef }) => {
  ...
  useEffect(() => {
    ...
    if (!portalRootRef.current) {
      console.warn("Tooltip portal root ('tooltip-portal-root') not found in the DOM!");
    }
  }, []);

  useEffect(() => {
    if (isVisible && targetRef.current && tooltipRef.current && portalRootRef.current) {
  ...
  return ReactDOM.createPortal(
    <div
      ref={tooltipRef}
      className="fixed z-[9999] w-64 rounded-md border border-gray-200 bg-white p-3 shadow-lg dark:border-gray-700 dark:bg-gray-800 transition-opacity duration-150"
      role="tooltip"
      style={{
    ...
    >
      <h4 className="mb-1.5 flex items-center text-sm font-semibold text-gray-900 dark:text-white">
        {TooltipIcon && <TooltipIcon className="mr-2 h-4 w-4 text-blue-500" />}
        {model.name}
      </h4>
      <p className="text-xs text-gray-600 dark:text-gray-300">{model.detailedDescription}</p>
    </div>,
