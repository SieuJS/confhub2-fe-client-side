...
const PersonalizationConfirmationModal: React.FC<PersonalizationConfirmationModalProps> = ({
  ...
  const getModalContent = () => {
    if (type === 'enableBenefit') {
      return {
        ...
        confirmText: t('Enable_Feature_Button') || "Enable",
  ...
  return (
    <Modal
      ...
      footer={
        <>
          {/* Nút "Cancel" hoặc "Update Profile" */}
          {type === 'missingInfo' ? (
            // Nếu type là 'missingInfo', sử dụng Link để điều hướng đến trang profile
            // và truyền query parameter 'tab=profile'
            <Link href={{
              pathname: '/dashboard',
              query: { tab: 'profile' }
            }}
              ...
              <button
                onClick={onClose} // Đóng modal khi nhấp vào nút "Update Profile"
              ...
              >
                {content.cancelText} {/* Sẽ hiển thị "Update Profile" */}
              </button>
            </Link>
          ) : (
            // Nếu type là 'enableBenefit', sử dụng nút thông thường để đóng modal
            <button
              onClick={onClose}
              className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
            >
              {content.cancelText} {/* Sẽ hiển thị "Cancel" */}
            </button>
          )}

          {/* Nút "Enable Anyway" hoặc "Enable" */}
          <button
            onClick={() => {
              onConfirm();
