...
import type {
  ...
  NavLinkItem,
  NavButtonItem
} from '@/src/hooks/regularchat/useLeftPanelNavigation'
...
const NavigationMenu: React.FC<NavigationMenuProps> = ({
  ...
  const commonButtonInteractiveClasses =
    'rounded-md  hover:bg-gray-100   dark:hover:bg-gray-800  focus:outline-none focus:ring-2 focus:ring-blue-500'
    ...
    <nav className={`${isLeftPanelOpen ? 'space-y-1' : 'space-y-1'}`}>
      {navItems.map(item => {
        ...
        const isActiveClasses =
          item.isActive && !itemIsActuallyDisabled // Không active nếu bị disabled
            ? 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300'
            : commonButtonInteractiveClasses
        const disabledClasses = itemIsActuallyDisabled
        ...
        if (item.type === 'link') {
          ...
          return (
            <Link
              ...
              aria-current={
                linkItem.isActive && !itemIsActuallyDisabled
                  ? 'page'
                  : undefined
              }
              aria-disabled={itemIsActuallyDisabled} // Sử dụng itemIsActuallyDisabled
              onClick={e => {
                if (itemIsActuallyDisabled) {
                  // Sử dụng itemIsActuallyDisabled
                  e.preventDefault()
                  return
                }
                if (linkItem.action) {
            ...
            >
              <IconComponent
                ...
                className={`${isLeftPanelOpen ? 'mr-3' : ''} flex-shrink-0 ${linkItem.isActive && !itemIsActuallyDisabled ? 'text-blue-600 dark:text-blue-400' : ' group-hover:text-gray-60  dark:group-hover:text-gray-30'}`}
                strokeWidth={1.75}
              />
        ...
        if (item.type === 'button') {
          ...
          return (
            ...
            >
              <IconComponent
                size={isLeftPanelOpen ? 20 : 24}
                className={`${isLeftPanelOpen ? 'mr-3' : ''} flex-shrink-0 ${buttonItem.isActive && !itemIsActuallyDisabled ? 'text-blue-600 dark:text-blue-400' : ' group-hover:text-gray-60  dark:group-hover:text-gray-30'}`}
                strokeWidth={1.75}
              />
