...
const DropZone: React.FC<DropZoneProps> = ({
  ...
  const bgColor = isDraggingOver ? 'bg-blue-50' : 'bg-gray-50'; // Lighter shades
  ...
  return (
    <div className="mb-3"> {/* Consistent margin */}
      <label className="mb-1 flex items-center text-sm font-medium text-gray-700"> {/* Darker label text, flex for alignment */}
        {label}
        {required && <span className="ml-1 text-red-500 font-semibold">*</span>} {/* Display asterisk if required */}
      </label>
      <div
      ...
      >
        {field ? (
          ...
          <div className="flex w-full items-center justify-between rounded border border-gray-300 bg-white p-1.5 px-2 shadow-sm">
            {/* Field Name and Type */}
            <span className={`mr-2 truncate text-sm font-medium ${field.type === 'dimension' ? 'text-blue-700' : 'text-green-700'}`}> {/* Slightly darker text */}
               {field.name}
               <span className="ml-1 text-xs font-normal text-gray-500">({field.type === 'dimension' ? 'Dim' : 'Measure'})</span>
            </span>
            {/* Remove Button */}
            <button
              onClick={(e) => {
              ...
              className="ml-1 flex-shrink-0 rounded p-0.5 text-gray-400 hover:bg-red-100 hover:text-red-600 focus:outline-none focus:ring-1 focus:ring-red-500 focus:ring-offset-1" // Added focus styles and padding
          ...
          <span className="text-center text-xs text-gray-500"> {/* Slightly darker placeholder */}
            {placeholderText}
          </span>
        )}
