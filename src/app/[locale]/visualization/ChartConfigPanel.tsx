...
import { CheckIcon, ChevronUpDownIcon, Cog6ToothIcon, EyeIcon, EyeSlashIcon, ChevronDoubleRightIcon } from '@heroicons/react/20/solid'; // Added ChevronDoubleRightIcon
...
const ChartConfigPanel: React.FC<ChartConfigPanelProps> = ({
    ...
    return (
        ...
        <div
            className={`
                relative flex-shrink-0 border-l border-gray-200 bg-white shadow-sm transition-all duration-300 ease-in-out
                ${isCollapsed ? 'w-0 overflow-hidden p-0 border-none' : 'w-72 h-full overflow-y-auto p-4'}
            `}
        >
                    ...
                    <div className="space-y-3">
                        ...
                        {showYAxis && (
                            <Droppable droppableId="yAxis">
                                {(provided, snapshot) => <DropZone id="yAxis" label={chartType === 'pie' ? "Value (Slice Size)" : "Y-Axis (Value/Measure)"} acceptedType="measure" field={getFieldById(config.yAxis?.fieldId)} onRemoveField={onRemoveField} provided={provided} snapshot={snapshot} required={true} />}
                            </Droppable>
                        )}
                        {showColor && (
