...
const VisualizationPage: React.FC = () => {
    ...
    const handleDragEnd = useCallback(
        (result: DropResult, provided: ResponderProvided) => {
            ...
            switch (targetZoneId) {
                case 'xAxis': acceptedType = chartConfig.chartType === 'scatter' ? 'any' : 'dimension'; break;
    ...
    const handleDownload = useCallback(() => {
        if (chartInstanceRef.current) {
            try {
                ...
                alert(`Failed to download chart: ${error instanceof Error ? error.message : String(error)}`);
            }
        } else {
    ...
    if (dataLoading) return <div className="flex h-screen w-screen items-center justify-center bg-gray-100"><Loading /></div>;
    if (dataError) return <div className="flex h-screen w-screen items-center justify-center bg-gray-100 p-4"><p className="rounded border border-red-300 bg-red-50 p-4 text-center text-red-700 shadow-md">Error loading visualization data: <br /> {dataError}</p></div>;
    if (!rawData) return <div className="flex h-screen w-screen items-center justify-center bg-gray-100 p-4"><p className="text-center text-gray-500">No data loaded or available for visualization.</p></div>;

    return (
        <DragDropContext onDragEnd={handleDragEnd} onDragStart={handleDragStart}>
            <div className="flex h-screen w-screen flex-row overflow-hidden bg-gray-100">
                <DataFieldPanel fields={availableFields} droppableId={FIELD_LIST_DROPPABLE_ID} isCollapsed={isFieldsPanelCollapsed} onToggle={toggleFieldsPanel} />
                <div className="relative flex flex-grow flex-col p-2">
                    {isFieldsPanelCollapsed && (<button onClick={toggleFieldsPanel} className="absolute left-0 top-1/2 z-20 -translate-y-1/2 transform rounded-r-md bg-gray-200 p-1 text-gray-600 shadow-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1" title="Expand Fields Panel" aria-label="Expand Fields Panel"><ChevronDoubleRightIcon className="h-5 w-5" /></button>)}
                    <ChartDisplay option={echartsOption} isLoading={dataLoading} isReady={isChartReady} config={chartConfig} onDownloadSvg={handleDownload} getChartInstance={handleGetChartInstance} chartHeight="100%" />
                    {isConfigPanelCollapsed && (<button onClick={toggleConfigPanel} className="absolute right-0 top-1/2 z-20 -translate-y-1/2 transform rounded-l-md bg-gray-200 p-1 text-gray-600 shadow-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1" title="Expand Configuration Panel" aria-label="Expand Configuration Panel"><ChevronDoubleLeftIcon className="h-5 w-5" /></button>)}
                </div>
                <ChartConfigPanel availableFields={availableFields} config={chartConfig} options={chartOptions} onConfigChange={updateChartConfig} onOptionsChange={updateChartOptions} onRemoveField={handleRemoveField} isCollapsed={isConfigPanelCollapsed} onToggle={toggleConfigPanel} />
            </div>
        </DragDropContext>
    );
};
