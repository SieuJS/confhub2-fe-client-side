...
const ConferenceFeedback: React.FC<ConferenceFeedbackProps> = ({
  ...
  const displayedFeedbacks = useProcessedFeedbacks(allFeedbacks, filterStar, sortOption);
  ...
  const handleSubmit = async () => {
    if (!conferenceId || star === null || description.trim() === '') {
      ...
      console.error('Missing conferenceId, star, or description', { conferenceId, star, description });
      return;
    }
          ...
          <div className='mb-8 space-y-4'>
            {paginatedFeedbacks.length === 0 ? (
              <div className='text-center text-gray-500'>
                {/* Adjust message based on filter/sort state */}
                {displayedFeedbacks.length > 0 ? `No feedback on this page.` :
                  filterStar !== null ? `No feedback matching ${filterStar} star${filterStar > 1 ? 's' : ''} found.` :
                    'No feedback yet.'}
              </div>
            ) : (
