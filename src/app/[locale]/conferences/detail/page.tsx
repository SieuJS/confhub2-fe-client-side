...
// import * as usersData from '../../../../../models/data/user-list.json';  // Import user data correctly
...
//   // --- Corrected users import WITH TYPE ASSERTION ---
//   const users: UserResponse[] = (usersData as any).default as UserResponse[] || usersData as UserResponse[];

//   useEffect(() => {
//     // Check if the conference is already followed on component mount
//     const userData = localStorage.getItem('user');
//     if (userData) {
//       const user = JSON.parse(userData);
//       if (user.followedConferences && Array.isArray(user.followedConferences) && user.followedConferences.length > 0) {
//         setIsFollowing(user.followedConferences.includes(conferenceData?.conference.id));
//       }
//     }
//   }, [conferenceData?.conference.id]);  // Depend on conferenceData.conference.id
...
//     if (userData) {
//       const user: { email: string; loginStatus: string; followedConferences?: string[] } = JSON.parse(userData); // Type assertion for clarity
//       const userEmail = user.email;
//       console.log(userEmail);
//       // --- Corrected user lookup ---
//       const userIndex = users.findIndex((u: UserResponse) => u.email === userEmail); // Cast u to UserResponse

//       if (userIndex !== -1) {
//         const updatedUser: UserResponse = { ...users[userIndex] };  // Use UserResponse type
...
//         if (isFollowing) {
//           // Unfollow: Remove the conference ID
//           updatedUser.followedConferences = updatedUser.followedConferences.filter(
//             (id: string) => id !== conferenceId  // Correctly compare IDs
//           );
//         } else {
//           // Follow: Add the conference ID if not already present
...
//         // Update localStorage (store only essential data, not the entire user object)
...
//       }
//     } else {
//       console.error("User not logged in"); // Or redirect to login page
//     }
//   };
...
//     return <NotFoundPage />;
//   }
//   if (!conferenceData) return <Loading />;
